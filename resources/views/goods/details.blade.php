				...
				<div class="shortcut">
					<ul class="fl">
						...
						<li class="f-item">请<a href="{{'/user/login'}}" {{session('user_name')}} target="_blank">登录</a>　
                            <span><a href="{{'/user/register'}}" target="_blank">免费注册</a></span>
                        </li>
                            @endif
					</ul>
					<ul class="fr">
					...
					<div class="yui3-u Right shopArea">
						<div class="fr shopcar">
							<div class="show-shopcar" id="shopcar">
								<span class="car"></span>
								<a class="sui-btn btn-default btn-xlarge" href="{{url('cart/index')}}" target="_blank">
									<span>我的购物车</span>
									<i class="shopnum">0</i>
								</a>
					...
					<div class="clearfix choose">
						...
						<div class="summary-wrap">
							...
							<div class="fl">
								...
								@if(session('uid'))
									<li>
                                        <a href="{{url('/cart/add')}}?goods_id={{$goods['goods_id']}}"  target="_blank" id="btn_cart" class="sui-btn  btn-danger addshopcar">加入购物车</a>
									</li>
                                 @else
                                        <li>
                                            <a href="{{url('/user/login')}}"  target="_blank" id="btn_cart" class="sui-btn  btn-danger addshopcar">加入购物车</a>
                                        </li>
									@endif

                                        @if($Collect ==1)
                                            <span href=""  target="_blank" id="fav" class="sui-btn  btn-danger addshopcar">不收藏</span>
                                        @else
                                        <span href=""  target="_blank" id="fav" class="sui-btn  btn-danger addshopcar">收藏</span>
                                    @endif
					...
					<div class="tab-main intro">
						...
						<div class="tab-content tab-wraped">
							...
							<div id="four" class="tab-pane">
                                <div>
                                    <textarea class="evaluate" cols="70" rows="8" placeholder="请输入你的评价"></textarea><br>
                                    <button class="btn-publish">发表</button>
                            </div>
                            </div>
						...
						<div class="tbar-panel-content J-panel-content">
							<div id="J-cart-tips" class="tbar-tipbox hide">
								<div class="tip-inner">
									...
									<a href="{{'/user/login'}}" class="tip-btn J-login">登录</a>
                                        @endif
								</div>
							</div>
...
<script>
    $(document).ready(function(){
        ...
        $(document).on("click","#fav",function (e) {
            ...
            e.preventDefault();
            $.ajax({
                url:"/goods/collect?id="+{{$goods['goods_id']}},
                type:'get',
                dataType:'json',
                success:function (d) {
                   if(d.error==200){
        ...
        $(document).on("click",".btn-publish",function () {
            var _this = $(this);// 当前点击的对象
            var _text = $(".evaluate");// 点击按钮触发的文本域
            var id = {{$goods['goods_id']}}; // 获取商品id
            if(_text.val()==''){
                alert('请输入评价');
                return false;
            }
            $.ajax({
                url:"/goods/comment?id={{$goods['goods_id']}}",
                data:'comment_value='+_text.val(),
                type:'post',
                dataType:'json',
            }).done(function (d) {
                if(d.error==200){
